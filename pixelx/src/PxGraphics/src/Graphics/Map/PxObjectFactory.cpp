...
static PxCube * CreateCube( const PxRuntimeObject & ro)
{
	...
	sscanf( ro.Params.c_str() , "%f,%f,%f,%f,%f,%f,%s" , &x,&x,&x,&x,&x,&x,g_buff);
	p->SetTexture(g_buff);
	return p;
}
...
{
	float a,b,c,d,e,f;
	sscanf( ro.Params.c_str() , "%f,%f,%f,%f,%f,%f,%s" , &a,&b,&c,&d,&e,&f,g_buff);
...
{
	float x,y,z;
	sscanf( ro.Params.c_str() , "%f,%f,%f" , &x,&y,&z);
...
static PxSteps * CreateSteps(const PxRuntimeObject & ro)
{
	float a,b,c,d,e,f;
	PxDirectionEnum de;
	sscanf( ro.Params.c_str() , "%d,%f,%f,%f,%f,%f,%f" , &de,&a,&b,&c,&d,&e,&f);
...
{
	float x,y,z,s;
	sscanf( ro.Params.c_str() , "%f,%f,%f,%f,%s" , &x,&y,&z,&s,g_buff);
