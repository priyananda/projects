...
void PxSteps::Register(PxPolygonCollection * pColl )
{
	switch(dir)
	{
		case eDirectionNorth:
			...
			{
				pColl->AddPolygon( std::make_unique<PxRectangle>(
					bBox.x,bBox.y + (i + 1)* stepHeight,bBox.z + i * stepHeight,
					length,0.0f,stepHeight,
					"WOOD3"
				));
				pColl->AddPolygon( std::make_unique<PxRectangle>(
					bBox.x,bBox.y + (i + 1)* stepHeight,bBox.z + i * stepHeight,
					length,-stepHeight, 0.0f,
					"WALL8"
				));
				pColl->AddPolygon( std::make_unique<PxRectangle>(
					bBox.x,bBox.y,bBox.z + i * stepHeight,
					0.0f,stepHeight * (i+1), stepHeight,
					"WOOD2"
				));
				pColl->AddPolygon( std::make_unique<PxRectangle>(
			...
			pColl->AddPolygon( std::make_unique<PxRectangle>(
				...
				length,numSteps * stepHeight, 0.0f,
				"WOOD2"
			));
			break;
		...
		case eDirectionWest:
			...
			pColl->AddPolygon( std::make_unique<PxRectangle>(
				bBox.x,bBox.y,bBox.z,
				0.0f,numSteps * stepHeight, length,
				"WOOD2"
			));
			break;
		case eDirectionEast:
