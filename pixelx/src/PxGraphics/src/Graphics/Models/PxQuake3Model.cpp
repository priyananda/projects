...
///////////////////////////////////////////////////////////////////////////////////
//
// This file includes all of the class definitions for PxQuake3ModelLoader and PxQuake3ModelCore.
...
// intimidated by it.
//
// Let me describe what these classes are for.  PxQuake3ModelCore is the class the 
// client (you) uses to load and draw the character.  PxQuake3ModelLoader is what the
// PxQuake3ModelCore class uses to load the character mesh information.  It is nearly
...
///////////////////////////////// CMODEL MD3 \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*
/////
/////	This is our PxQuake3ModelCore constructor
/////
///////////////////////////////// CMODEL MD3 \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*

PxQuake3ModelCore::PxQuake3ModelCore(cstrref pathname)
{
	...
	//this->LoadWeapon( (LPSTR)pathname.c_str() , "RailGun" );
}


///////////////////////////////// ~CMODEL MD3 \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*
/////
/////	This is our PxQuake3ModelCore deconstructor
/////
///////////////////////////////// ~CMODEL MD3 \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*

PxQuake3ModelCore::~PxQuake3ModelCore()
{
...
void PxQuake3ModelCore::DestroyModel(t3DModel *pModel)
{
...
bool PxQuake3ModelCore::LoadModel(
	char *strLowerModel,
	char *strUpperModel,
	char *strHeadModel,
	char *strLowerSkin,
	...
	// This stores the file name for the head.md3 skin
	PxQuake3ModelLoader loadMd3;				// This object allows us to load each .md3 and .skin file

	// This function is where all the character loading is taken care of.  We use
	// our PxQuake3ModelLoader class to load the 3 mesh and skins for the character. Since we
	// just have 1 name for the model, we add that to _lower.md3, _upper.md3 and _head.md3
	// to load the correct mesh files.

	// Next we want to load the character meshes.  The PxQuake3ModelCore class has member
	...
	if(!loadMd3.ImportMD3(&m_Head,  strHeadModel))
	{
		...
		PxLog::LogMessage("Unable to load the HEAD model!");
		return false;
	}
	...
	{
		// Display an error message telling us the file could not be found
		PxLog::LogMessage("Unable to load the UPPER model!");
		return false;
	}
	...
	{
		// Display an error message telling us the file could not be found
		PxLog::LogMessage("Unable to load the LOWER model!");
		return false;
	}
	...
	{
		// Display an error message telling us the file could not be found
		PxLog::LogMessage("Unable to load the LOWER skin!");
		return false;
	}
	...
	{
		// Display an error message telling us the file could not be found
		PxLog::LogMessage("Unable to load the UPPER skin!");
		return false;
	}
	...
	{
		// Display an error message telling us the file could not be found
		PxLog::LogMessage("Unable to load the HEAD skin!");
		return false;
	}
	...
	LoadModelTextures(&m_Lower, texBaseDir);
	LoadModelTextures(&m_Upper, texBaseDir);
	LoadModelTextures(&m_Head,  texBaseDir);
...
bool PxQuake3ModelCore::LoadWeapon(LPSTR strPath, LPSTR strModel)
{
	...
	PxQuake3ModelLoader loadMd3;					// This object allows us to load the.md3 and .shader file
	...
	if(!loadMd3.ImportMD3(&m_Weapon,  strWeaponModel))
	{
		...
		PxLog::LogMessage("Unable to load the WEAPON model!");
		return false;
	}
	...
	if(!loadMd3.LoadShader(&m_Weapon, strWeaponShader))
	{
		...
		PxLog::LogMessage("Unable to load the SHADER file!");
		return false;
	}
...
void PxQuake3ModelCore::LoadModelTextures(t3DModel *pModel, LPSTR strPath)
{
	...
	for(int i = 0; i < pModel->numOfMaterials; i++)
	{
		...
		if(strlen(pModel->pMaterials[i].strFile) > 0)
		{
			...
			m_Textures[strTextures.size()] = PxTextureManager::LoadTexture( strFullPath,strFullPath);								
...
void  PxQuake3ModelCore::LinkModel(t3DModel *pModel, t3DModel *pLink, LPSTR strTagName)
{
...
void PxQuake3ModelCore::DrawModel()
{
	// This is the function that is called by the client (you) when using the 
	// PxQuake3ModelCore class object.  You will notice that we rotate the model by
...
void PxQuake3ModelCore::DrawLink(t3DModel *pModel)
{
	...
	for(int i = 0; i < pModel->numOfTags; i++)
	{
		...
		{			
			...
			PxVector vPosition = pModel->pTags[i].vPosition;
...
void PxQuake3ModelCore::RenderModel(t3DModel *pModel)
{
	...
	for(int i = 0; i < pModel->numOfObjects; i++)
	{
		...
		if(pObject->bHasTexture)
		{
			...
			int tid = pModel->pMaterials[pObject->materialID].texureId;
		...
		glBegin(GL_TRIANGLES);
			...
			for(int j = 0; j < pObject->numOfFaces; j++)
			{
				...
				for(int whichVertex = 0; whichVertex < 3; whichVertex++)
				{
					...
					PxVector vPoint1 = pObject->pVerts[ index ];
...
PxQuake3ModelLoader::PxQuake3ModelLoader()
{
...
bool PxQuake3ModelLoader::ImportMD3(t3DModel *pModel, char *strFileName)
{
...
void PxQuake3ModelLoader::ReadMD3Data(t3DModel *pModel)
{
	int i = 0;
...
void PxQuake3ModelLoader::ConvertDataStructures(t3DModel *pModel, tMd3MeshInfo meshHeader)
{
	int i = 0;
	...
	currentMesh.pVerts    = new PxVector [currentMesh.numOfVerts * meshHeader.numMeshFrames];
...
bool PxQuake3ModelLoader::LoadSkin(t3DModel *pModel, LPSTR strSkin)
{
...
bool PxQuake3ModelLoader::LoadShader(t3DModel *pModel, LPSTR strShader)
{
	// Make sure valid data was passed in
...
void PxQuake3ModelLoader::CleanUp()
{
...
void PxQuake3ModelCore::Animate( int startFr , int endFrame ,float interp)
{
	this->DrawModel();
}

void PxQuake3ModelCore::Render()
{
	this->DrawModel();
}
/////////////////////////////////////////////////////////////////////////////////
