quizRunnerModule.factory('Storage', function($window) {
  ...
  helper.getPerson = function() {
    var personAsString = $window.localStorage.getItem('hph-person');
  ...
  helper.savePerson = function(person) {
    $window.localStorage.setItem('hph-person', JSON.stringify(person));
  };
  
  helper.getScore = function() {
    var scoreAsString = $window.localStorage.getItem('hph-score');
    var ret = new Map();
  ...
  helper.saveScore = function(score) {
    $window.localStorage.setItem('hph-score', JSON.stringify(Array.from(score.keys())));
  };
